...
func configure_new_session(seed: String = "", new_preferences := {}, new_game_state := {}) -> void:
    _rng.randomize()
    random_seed = seed if seed != "" else str(_rng.randi())
    preferences = new_preferences.duplicate(true)
    game_state = new_game_state.duplicate(true)
    session_start_ms = OS.get_ticks_msec()
    session_id = 0
    ...
    _session_request.request(SESSION_ENDPOINT, [], true, HTTPClient.METHOD_POST, to_json(payload))
...
func _post_command(command_type: String) -> void:
    var timestamp_ms = OS.get_ticks_msec() - session_start_ms
    var payload := {
        "session": session_id,
        "command_type": command_type,
        "timestamp_ms": timestamp_ms
    }
    _command_request.request(COMMAND_ENDPOINT, [], true, HTTPClient.METHOD_POST, to_json(payload))

func _on_session_request_completed(result: int, response_code: int, headers: Array, body: PackedByteArray) -> void:
...
func _on_command_request_completed(result: int, response_code: int, headers: Array, body: PackedByteArray) -> void:
