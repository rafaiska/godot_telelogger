...
Play Command Logger is a lightweight Django + Django REST framework service that lets you record each play session together with the sequence of entity commands issued during that session. It models a session’s metadata (random seed, preferences, game state) and the list of timestamped commands so you can later analyze what happened within a playthrough.
...
- `POST /api/sessions/` – create a new play session with optional nested commands. The payload must include `random_seed` and may include `preferences`, `game_state`, and `commands`. Each command needs `entity_id`, `entity_state`, `command_type`, and `timestamp_ms`. Example:
  ```json
  {
...
- `POST /api/commands/` – create a command manually (`session`, `entity_id`, `entity_state`, `command_type`, `timestamp_ms` required).
...
- `playlogs.models` defines `PlaySession` and `EntityCommand` along with sensible default ordering (`created_at` descending for sessions, `timestamp_ms` ascending for commands).
